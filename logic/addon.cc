...
NAN_METHOD(FindBestLead) {
  ...
  string fline(*NanAsciiString(args[0])); 
  ...
  leader = args[1]->Uint32Value();
  string sContract = (*NanAsciiString(args[2]));
   char *contract  = new char[sContract.length() + 1];
   strcpy(contract, sContract.c_str());
  infname = "test.pbn";

  if (showVersion) {
    fprintf(stderr, "\nleadsolver %s (compiled %s %s)\n", VER_STR, __DATE__, __TIME__);
    if (nonSwitchCnt == 0) { NanReturnValue(NanNew<Number>(SUCCESS));  }
  }
  if (!contract) {
    fprintf(stderr, "No contract specified.\n");
    NanReturnValue(NanNew<Number>( ERR_ARG_PARSING));
  }
  if (!infname) {
  ...
  if (!hDLL) {
    ...
    LocalFree(lpMsgBuf);
    NanReturnValue(NanNew<Number>( ERR_DDS_LOAD_FAILED));
  }
  ...
  if (!hFunc) {
    ...
    LocalFree(lpMsgBuf);
    NanReturnValue(NanNew<Number>(ERR_NO_SolveAllBoards));
  }
  ...
  if (outfname) {
    if ( (ofd = fopen(outfname, "w")) == NULL ) {
      cerr << "Unable to open/write file: " << outfname << endl; NanReturnValue(NanNew<Number>( ERR_BAD_OUTPUT_FILE));
    }
  }
  else {
    ofd = stdout;
  ...
  // 	result.lead[0] = suitrank[suit];
  // 	result.lead[1] = cardrank[rank];
  	char lead[3];
  	lead[0] = suitrank[suit];
  	lead[1] = ' ';
  	lead[2] = cardrank[rank];
  	Local<String> temp = NanNew(lead);
  	result.nunmberMade = lx->avgTricks;
...
void Init(Handle<Object> exports, Handle<Object> module) {
  NODE_SET_METHOD(module, "exports", FindBestLead);
}
